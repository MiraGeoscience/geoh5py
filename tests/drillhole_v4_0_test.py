...
def test_concatenator(tmp_path):
    ...
    with Workspace(version=2.0).save(h5file_path) as workspace:
        # Create a workspace
...
def test_create_drillhole_data(tmp_path):  # pylint disable=R0915
...
def create_drillholes(h5file_path, version=1.0, ga_version="1.0"):
    ...
    with Workspace(version=version, ga_version=ga_version).save(
        h5file_path
    ) as workspace:
        # Create a workspace
