...
from geoh5py.ui_json.forms import BaseForm, FormParameter
...
class UIJson(BaseModel):
    ...
    @field_validator("geoh5", "workspace", mode="before")
    @classmethod
    def path_to_workspace(cls, path):
    ...
    def to_params(self) -> dict[str, Any]:
        """Promote, validate, and flatten parameter/values dictionary."""

        with self.geoh5.open(mode='r') as workspace:
            params = {}
            for field in self.model_fields_set:
                value = getattr(self, field)
                value = value.flatten() if isinstance(value, BaseForm) else value
                value = workspace.get_entity(value)[0] if isinstance(value, UUID) else value
                params[field] = value
