...
class Points(ObjectBase):
    ...
    @vertices.setter
    def vertices(self, xyz: np.ndarray | None):
        if getattr(self, "_vertices", None) is not None:
            raise UserWarning(
                "Attempting to re-assign 'vertices'. "
                "Consider using the `remove_vertices` method or create a new entity."
            )

        if xyz is None and self.on_file:
            xyz = self.workspace.fetch_array_attribute(self, "vertices")
        ...
        elif xyz.dtype != np.dtype([("x", "<f8"), ("y", "<f8"), ("z", "<f8")]):
            raise ValueError(
                "Array of vertices must be of shape (*, 3) or a recarray with "
                "dtype=[('x', '<f8'), ('y', '<f8'), ('z', '<f8')]. "
                f"Array of shape {xyz.shape} provided"
            )
