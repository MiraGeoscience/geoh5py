        ...
        if export.value:
            ...
            if file_type.value == 'xyz':
                ...
                export_curve_2_shapefile(
                    ...
                    file_name=export_as.value,
                    epsg=epsg_code.value
                )
                print(f"Object saved to {export_as.value}.shp")

    def update_options(_):

        if file_type.value == 'ESRI shapefile':
...
def contour_values_widget(h5file, contours=""):
    ...
    return widgets.HBox([out, VBox([export_as, z_value, export])])
...
def em1d_inversion_widget(h5file, plot_profile=True, start_channel=None):
