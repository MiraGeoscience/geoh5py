...
class CoordList(object):
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('CoordList')
        if self.coords is not None:
            oprot.writeFieldBegin('coords', TType.LIST, 1)
            oprot.writeListBegin(TType.LIST, len(self.coords))
            for iter12 in self.coords:
...
all_structs.append(CoordList)
CoordList.thrift_spec = (
    None,  # 0
    (1, TType.LIST, 'coords', (TType.LIST, (TType.DOUBLE, None, False), False), None, ),  # 1
)
